...
def read_sdt_openscan(filename,print_on=False):
    ...
    with open(filename,'rb') as fid:
        ...
        for measblock_index in range(header.no_of_meas_desc_blocks):
            #fmeas_blk = fid.read()
            mblocks.append(np.rec.fromfile(fid, dtype=np.dtype(sdtfile.sdtfile.MEASURE_INFO), shape=1, byteorder='<'))
            fid.seek(header.meas_desc_block_length - np.dtype(sdtfile.sdtfile.MEASURE_INFO).itemsize, 1)
